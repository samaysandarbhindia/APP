...
export default function MasterKeysPage({ ctx }) {
  ...
  const canManageKeys = Boolean(ctx.access?.canManageKeys);
  const denyKeys = () => notify(ctx.access?.denied?.('manage provider keys') || 'Your role does not allow this action.', 'error');
  ...
  const saveMasterKey = async () => {
    if (!canManageKeys) return denyKeys();
